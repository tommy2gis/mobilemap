 ...
 * @Last Modified by: 史涛
 * @Last Modified time: 2020-04-24 17:27:58
 */
...
function loginout() {
  if (loginCas) {
    ...
    //var url = "http://localhost:8080/portal/logout.jsp";
...
function checkToken() {
  const token = getCookie("userinfo")
    ? JSON.parse(getCookie("userinfo")).geokey
    : null;
  if (token) {
    return axios
      .get(ServerUrl + "/core/system/get_info", {
        params: {
            token: token,
            system: "datacenter"
        }
...
function delCookie(name) {
  ...
  Cookies.remove(name, { path: "/portal" });
